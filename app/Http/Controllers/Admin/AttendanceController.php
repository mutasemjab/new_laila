...
class AttendanceController extends Controller
{
    ...
    public function room($room_id)
    {
        ...
        $users =  User::with(['latestAttendance' => function($query) use ($room_id,$room) {
            if(!$room->is_main) $query->where('room_id', $room_id);
        }])->whereHas('latestAttendance',function ($query) use($room_id,$room){
            if(!$room->is_main) $query->where('room_id', $room_id);
    ...
    public function getRoomUsers(Request $request,$room_id)
    {
        ...
        ->whereHas('latestAttendance',function ($query) use($room_id,$room,$request){
            if(!$room->is_main) $query->where('room_id', $room_id);
